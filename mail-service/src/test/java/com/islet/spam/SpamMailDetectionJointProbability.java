...
public class SpamMailDetectionJointProbability {
    ...
    public static void main(String[] args) throws IOException {
        SpamMailDetectionJointProbability smc = new SpamMailDetectionJointProbability();
        smc.TrainMail();
        smc.TestMail();
        smc.evaluationModel();
    }
            ...
            if (nowIndex < startCount) {
                ...
                for (String s : wordList) {
                    if (spamMailRateMap.containsKey(s)) {
                        double tmp = spamMailRateMap.get(s);

                        if (testNum == 116 || testNum == 236||testNum == 404){
                        ...
                        if (tempRate == 0d && rate == 0d){//同时趋于0,则概率设为0.5
                            ...
                            flag = 2;//正常趋于0
                            if (rate > Math.pow(10, -200)){//差125量级，直接判定
                                break;
                            }
                        }else if(rate == 0d){//垃圾邮件趋于0
                            probability[testNum] = 0d;
                            flag = 3;//垃圾趋于0
                            if (tempRate > Math.pow(10, -200)){//差125量级，直接判定
                                break;
                            }
                        }
                        if (flag == 1){//已经同时趋于0，不需要继续遍历
                ...
                if (probability[testNum] > 0.9 && type.equals("ham")) {
    ...
    private void evaluationModel() {
        ...
        String probabilityFilePath = "probability-JointProbability.jpg";
        ...
        String ROCFilePath = "ROC-JointProbability.jpg";
        //创建Dataset对象
